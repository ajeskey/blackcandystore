...
- **Audiobook & live-music enrichment** _(Store)_ — content is identified from your files/tags first, then validated and enriched from the right source: [Open Library](https://openlibrary.org) for audiobooks (author, publication year, cover art; no key) and [setlist.fm](https://api.setlist.fm/) for live recordings (setlist validation, venue, event date; free non-commercial key). Ordinary music continues to use Discogs. See [docs/metadata-enrichment.md](docs/metadata-enrichment.md).
...
| BROADCASTER_URL              | http://127.0.0.1:9340 | Base URL of the broadcaster service that assembles the always-on radio / co-listen MP3 streams and fans them out to listeners. Defaults to a co-located broadcaster on loopback (mirrors `PLAYBACK_SIDECAR_URL`). Rails owns all sequencing/auth and reverse-proxies listeners to the broadcaster; if it is not running, radio and co-listen streaming are unavailable while the rest of the app is unaffected. See [broadcaster/README.md](broadcaster/README.md). |
